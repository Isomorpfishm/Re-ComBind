...
for i in range(len(pro_list)):
    ...
    for j in range(4):
        k = 4 * i + j
        lig_files = glob.glob(path + '/vina/' + trial + '/output_filter/' + 
        ...
        for l in range(len(lig_files)):
            ...
            with open (trial + '_reference_bitstring.txt', 'a') as fo:
...
fps.to_csv(path + '/' + trial + '_reference_descriptors.csv', index=False)



"""
protein = MolFromPDBFile('3ui7.pdb')
ligand = MolFromPDBFile('4llx.pdb')

protein_addH = AddHs(protein, addCoords = True, addResidueInfo = True)
ligand_addH = AddHs(ligand, addCoords = True)

protein_donors, protein_acceptors = init_hbond(protein_addH)
ligand_donors, ligand_acceptors = init_hbond(ligand_addH)

hbonds = _hbond_compute(ligand_donors, protein_acceptors, settings, False)
print(hbonds)
df1 = pd.DataFrame(hbonds)
print(df1)
print('\n')

hbonds = _hbond_compute(protein_donors, ligand_acceptors, settings, True)
print(hbonds)
...
mols = MolFromPDBFile('3ui7.pdb')
mols = AddHs(mols, addResidueInfo=True, addCoords=True)
protein = Molecule(mols, True, settings)

mols = MolFromPDBFile('4llx.pdb')
mols = AddHs(mols, addCoords=True)
ligand = Molecule(mols, False, settings)
